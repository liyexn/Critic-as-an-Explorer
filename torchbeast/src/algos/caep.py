...
MiniHackInverseDynamicsAttentionNet = models.MiniHackInverseDynamicsAttentionNet
...
def learn(actor_model,
    ...
    with lock:
        ...
        icm_state_emb = learner_outputs['emb'][:-1]
        icm_next_state_emb = learner_outputs['emb'][1:]
        prediction_actions = inverse_dynamics_model(icm_state_emb, icm_next_state_emb)
        inverse_dynamics_loss = losses.compute_inverse_dynamics_loss(prediction_actions, batch['action'][1:])
        ...
        # running normalization to the intrinsic or extrinsic reward
        ...
        for param_1, param_2 in zip(model.parameters(), elliptical_encoder.parameters()):
            if param_1.size() != param_2.size():
                break
            param_2.data.copy_(param_1.data)
            param_2.requires_grad = False

        elliptical_encoder.weighted_emb.weight.data.copy_(inverse_dynamics_model.weighted_emb.weight.data)
        elliptical_encoder.weighted_emb.requires_grad = False
...
def train(flags):
    ...
    xpid += f'-dim_{flags.weighted_emb_size}'
    ...
    if 'MiniHack' in flags.env:
        ...
        elliptical_encoder = NetHackStateEmbeddingNet(env.observation_space, False, weighted_emb_size=flags.weighted_emb_size)
        encoder_parameter = sum(p.numel() for p in elliptical_encoder.parameters())

        inverse_dynamics_model = MiniHackInverseDynamicsAttentionNet(
            env.action_space.n,
            emb_size=1024,
            weighted_emb_size=flags.weighted_emb_size
        ).to(device=flags.device)

        idn_parameter = sum(p.numel() for p in inverse_dynamics_model.parameters())

        print('----------- model_parameter:', model_parameter)
        print('----------- encoder_parameter:', encoder_parameter)
        print('----------- idn_parameter:', idn_parameter)
